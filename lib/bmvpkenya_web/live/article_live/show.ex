defmodule BmvpkenyaWeb.ArticleLive.Show do
  ...
  defp get_price_in_cents(article) do
    {:KES, custom_price, -2, _rest} = Money.to_integer_exp(article.price)
    custom_price
  end
  ...
  # def is_author?(nil, article), do: false
  ...
  defp has_valid_token?(%{"token" => token} = _params, article) do
    case BmvpkenyaWeb.UrlHelper.verify_unique_article_token(token) do
